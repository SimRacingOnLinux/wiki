...
4. Create a folder to contain your Wine prefix, for these docs I will be using `$HOME/wine/simucube` but you are free to substitute your own path.
```shell
...
7. Now we need to change some registries to disable SDL and Input in Wine by running the following in a terminal:
```shell
WINEPREFIX="$HOME/wine/simucube"
wine reg add "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\winebus" /v "Enable SDL" /t REG_DWORD /d 0
wine reg add "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\winebus" /v DisableInput /t REG_DWORD /d 1
```
8. At this point we can run the installer for True Drive by running this command:
```shell
